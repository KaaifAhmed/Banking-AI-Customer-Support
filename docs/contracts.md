      ...
      "transactions": [
        {
          ...
          "description": "Funds transfer to Meezan Account ****4821",
...
- **Description:** Lists registered utility billers and unpaid/recent bills.
- **Auth Scope:** `Customer (Owner only)`
- **Success Response (`200 OK`):**
  ```json
  {
...
### `GET /api/jobs/{job_id}/`
- **Description:** Polls the asynchronous status and result of a processed AI job.
- **Auth Scope:** `IsAuthenticated`
...
- **Description:** Full-system operational health summary across database, Redis queues, worker heartbeats, and WhatsApp gateway.
...
- **Success Response (`200 OK`):**
  ```json
  {
    ...
    "data": {
      "status": "HEALTHY",
      "components": {
        "postgres": { "status": "UP", "active_connections": 14 },
        "redis": { "status": "UP", "used_memory_human": "2.4M" },
        "ai_worker_pool": { "status": "UP", "active_replicas": 3, "queue_depth": 1 },
        "background_worker": { "status": "UP", "queue_depth": 0 },
        "whatsapp_service": { "status": "UP", "session_authenticated": true }
      },
...
- **Description:** Aggregated AI cost, token usage, latency percentiles, and guardrail trigger stats.
- **Auth Scope:** `Staff (SystemAdmin Group)`
- **Success Response (`200 OK`):**
  ```json
  {
...
- **Success Response (`200 OK`):**
  ```json
  {
    ...
    "data": {
      ...
      "system_prompt": "You are Meezan Bank's official AI Support Assistant...",
      "model_tiers": {
...
- **Request Body (Media / PDF Statement):**
  ```json
  {
    ...
    "filename": "Meezan_Account_Statement.pdf"
  }
  ```
- **Success Response (`200 OK`):**
  ```json
...
## 10. LangGraph Scoped Tool Signatures (AI Worker)

Every tool callable by LangGraph nodes is bound to explicit typed parameters:

```python
# ai-worker/tools.py

@tool
def get_balance_tool(account_id: Optional[str] = None) -> dict:
    """Retrieves available and current balance for customer's account."""
    # Invokes Main Service GET /api/banking/accounts/{id}/balance/
...
def list_transactions_tool(
    ...
    """Queries and filters customer transaction history."""
    # Invokes Main Service GET /api/banking/transactions/?...

@tool
def explain_transaction_tool(transaction_id: str) -> dict:
    """Retrieves full merchant and fee explanation for a specific transaction."""
    # Invokes Main Service GET /api/banking/transactions/{id}/

@tool
def stage_transfer_tool(recipient_name: str, amount: float) -> dict:
    """Stages a funds transfer and returns confirmation preview. Does NOT move money."""
    # Invokes Main Service POST /api/actions/stage-transfer/

@tool
def confirm_transfer_tool(pending_action_id: str) -> dict:
    """Executes a staged transfer after customer gives explicit confirmation."""
    # Invokes Main Service POST /api/actions/confirm-transfer/

@tool
def pay_bill_tool(biller_name: str) -> dict:
    """Fetches bill dues and executes payment upon user confirmation."""
    # Invokes Main Service POST /api/actions/pay-bill/

@tool
def set_card_status_tool(card_id: str, status: str, reason: str) -> dict:
    """Sets card status to FROZEN, ACTIVE, or BLOCKED_LOST."""
    # Invokes Main Service POST /api/actions/card-status/

@tool
def lodge_dispute_tool(transaction_id: str, reason: str, auto_freeze_card: bool = True) -> dict:
    """Lodges an official dispute ticket and auto-offers card freeze."""
    # Invokes Main Service POST /api/actions/dispute/

@tool
def verify_cnic_tool(cnic_last4: str) -> dict:
    """Verifies last 4 digits of CNIC for WhatsApp onboarding."""
    # Invokes Main Service POST /api/actions/verify-cnic/
```

---

## 11. Traceability & Lock Status
...
- **Team Impact:**
  - **Areeb (Frontend):** May immediately mock these responses to build all pages.
  - **Hamza & Kaaif (Backend):** Implement DRF serializers/views strictly matching these shapes.
  - **Javed & Kaaif (AI Worker):** Implement LangGraph tools matching these parameter schemas.
